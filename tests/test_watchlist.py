...
tests/test_collection.py (see Comment 3 in the review).
"""
...
def test_add_to_watchlist_visibility(app, sample_user, sample_film):
    """
    Entries are private (public=False) by default; callers can opt into
    visibility by passing public=True.
    """
    with app.app_context():
        default_entry = add_to_watchlist(user_id=sample_user, film_id=sample_film)
        assert default_entry.public is False

        # A second user makes the same film public.
